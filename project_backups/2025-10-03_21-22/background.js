...
async function showNotification(alertData, tab) {
  try {
    // Use chrome.runtime.getURL to get the correct path to the icon
    const iconPath = chrome.runtime.getURL('icons/icon48.svg');
    ...
    chrome.notifications.create(
      ...
      (notificationId) => {
        if (chrome.runtime.lastError) {
          console.error('[background] Error showing notification:', chrome.runtime.lastError);
        } else {
